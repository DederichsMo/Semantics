...
\large
	...
	\indent\indent To shorten the derivation tree we first introduce the  following two abbreviations.\\
	\indent\indent $c_{1}=\text{\textbf{while}}\; \left(x \leq y\right)\; \text{\textbf{do}}\; c_{2}\; \text{\textbf{end}}$\\
	\indent\indent $c_{2}= y\;:=\;y\;-\;x;\; x\;:=\;x\;-\;4$\\

	\indent Furthermore we introduce the notation $\sigma_{ij}$ which defines $\sigma \left(x\right)\;=\;i$ and $\sigma \left(y\right)\;=\;j$.\\
	\begin{center}		
		\begin{prooftree}
